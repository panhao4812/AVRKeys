#include "../Functions.h"
#include "../ws2812.h"
#if (defined xd60 )||  (defined xd75 )||  (defined PG60 )
///////////////////////////////////////////////////////////////////////////
#if defined PG60
/*
...
volatile uint16_t rgb_rainbow[WS2812_COUNT]={0,34,68,102,136,170,205,240,205,170,136,102,68,34,0};
...
void Rainbow_LED(uint16_t delay_val_blink){
	for(uint8_t i=0;i<WS2812_COUNT;i++){
		...
		if(delay_val_blink==MAX_DELAY_4){C_r=pgm_read_byte(Rcolors+rgb_rainbow[i]);ws2812SetR(i,C_r);}
		if(delay_val_blink==MAX_DELAY_3){C_g=pgm_read_byte(Gcolors+rgb_rainbow[i]);ws2812SetG(i,C_g);}
		if(delay_val_blink==MAX_DELAY_2){C_b=pgm_read_byte(Bcolors+rgb_rainbow[i]);ws2812SetB(i,C_b);}
